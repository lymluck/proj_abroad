...
public class SuspensionDecoration extends RecyclerView.ItemDecoration {
    ...
    private static int COLOR_TITLE_FONT = Color.WHITE;
    ...
    public SuspensionDecoration setPic(boolean pic) {
        this.isPic = pic;
        mTitleHeight = mContext.getResources().getDrawable(R.drawable.ic_top).getIntrinsicHeight();
        return this;
    }
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (position > -1) {
                if (position == 0) {//等于0肯定要有title的
                    drawGroupHeaderArea(c, left, right, child, params, position);
                } else {//其他的通过判断
...
/*
    ...
    private void drawGroupHeaderArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        if (isPic) {
            Drawable drawable = mContext.getResources().getDrawable(mDatas.get(position).drawableId());
            if (drawable != null) {
                drawable.setBounds(left, child.getTop() - params.topMargin - mTitleHeight, right, child.getTop() - params.topMargin);
                drawable.draw(c);
            }
        } else {
    ...
    public void onDrawOver(Canvas c, final RecyclerView parent, RecyclerView.State state) {//最后调用 绘制在最上层
        ...
        if (!isPic) {
            ...
            Drawable drawable = mContext.getResources().getDrawable(mDatas.get(pos).drawableId());
            if (drawable != null) {
                drawable.setBounds(parent.getPaddingLeft(), parent.getPaddingTop(), parent.getRight() - parent.getPaddingRight(), parent.getPaddingTop() + mTitleHeight);
                drawable.draw(c);
            }
        }
        if (flag)
