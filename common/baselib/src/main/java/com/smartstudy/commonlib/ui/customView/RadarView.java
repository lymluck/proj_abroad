...
import com.smartstudy.commonlib.utils.AnimeUtils;
import com.smartstudy.commonlib.utils.RotateUtils;
...
public class RadarView extends View {
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mRadarBgColor = typedArray.getColor(R.styleable.RadarView_radar_bg_color, Color.TRANSPARENT);
        typedArray.recycle();
    }

    private void init() {
        mRadarPath = new Path();
        mAnimeUtil = new AnimeUtils(this);
    ...
    public void animeValue(int duration, RadarData data) {
        if (!mAnimeUtil.isPlaying(data)) {
            mAnimeUtil.animeValue(AnimeUtils.AnimeType.ZOOM, duration, data);
        }
    }
...
//        animeValue(1000, data);
    ...
    private void handleRotate(double rotate) {
        rotate = RotateUtils.getNormalizedAngle(rotate);
        mRotateAngle = rotate;
        invalidate();
    }
