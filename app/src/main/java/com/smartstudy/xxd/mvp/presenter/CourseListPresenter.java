...
public class CourseListPresenter implements CourseListContract.Presenter {
    ...
    @Override
    public void getCourses(int page, final int request_state) {
        CourseModel.getCourses(ParameterUtils.NETWORK_ELSE_CACHED, page, false, new BaseCallback<String>() {
            @Override
            ...
            public void onSuccess(String result) {
                List<CourseGroup> data = JSON.parseArray(result, CourseGroup.class);
                List<CourseGroup.Course> courses = new ArrayList<>();
                if (data != null) {
                    for (CourseGroup group : data) {
                        int len = group.getList().size();
                        for (int i = 0; i < len; i++) {
                            if (i == len - 1) {
                                group.getList().get(i).setBottom(true);
                                group.getList().get(i).setTop(false);
                            } else {
                                group.getList().get(i).setBottom(false);
                                if (i == 0) {
                                    group.getList().get(i).setTop(true);
                                } else {
                                    group.getList().get(i).setTop(false);
                                }
                            }
                        }
                        courses.addAll(group.getList());
                    }
                    view.showCourses(courses, request_state);
                    data = null;
                }
            }
        });
