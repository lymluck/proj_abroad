...
public class SpecialListActivity extends BaseActivity implements SpecialListContract.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        addActivity(this);
        UApp.actionEvent(this, "13_B_professional_list");
    }
    ...
    public void initEvent() {
            ...
            public boolean handleMessage(Message msg) {
                switch (msg.what) {
                    ...
                    case ParameterUtils.MSG_WHAT_FINISH:
                        ...
                        mClickPos = position;
                        ((TreeItemGroup) treeRecyclerAdapter.getData(position)).setCollapseOthers(true);
                        ((TreeItemGroup) treeRecyclerAdapter.getData(position)).setExpand(true);
                        if (treeRecyclerAdapter.getData(position).getItemManager() == null) {
                            treeRecyclerAdapter.getData(position).setItemManager(treeRecyclerAdapter.getItemManager());
                        }
                        ((TreeItemGroup) treeRecyclerAdapter.getData(position)).notifyExpand();
                        mHandler.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                // 指定postion滚动到顶部
                                linearLayoutManager.scrollToPositionWithOffset(position + mHeaderAdapter.getHeadersCount(), 1);
                                updateSuspensionBar();
                            }
                        }, 100);
                        break;
        ...
        searchView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                UApp.actionEvent(SpecialListActivity.this, "13_A_professional_search_btn");
        ...
        mHeaderAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickLitener() {
            ...
            public void onItemClick(ViewHolder viewHolder, BaseItem baseItem, int itemPosition) {
                if (baseItem instanceof TreeItemGroup) {
                    ...
                    if ("home".equals(flag)) {
                        UApp.actionEvent(SpecialListActivity.this, "13_A_professional_cell");
                        ...
                        toMoreDetails.putExtra(WEBVIEW_URL, String.format(HttpUrlUtils.getWebUrl(HttpUrlUtils.WEBURL_SPE_INTRO), data.getId()));
        ...
        if (mCurrentPosition < treeRecyclerAdapter.getItemCount()) {
            ...
            if (item instanceof TreeItemGroup) {
                if (item.getData() instanceof ProgramInfo) {
                    tvTypeName.setText(((ProgramInfo) item.getData()).getName());
                } else if (item.getData() instanceof MajorInfo) {
                    tvTypeName.setText(((MajorInfo) item.getData()).getName());
                }
                if (((TreeItemGroup) item).isExpand()) {
                    if (mSuspensionBar.getVisibility() == View.INVISIBLE) {
                        mSuspensionBar.setVisibility(View.VISIBLE);
