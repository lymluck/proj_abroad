...
public class RankTypeActivity extends BaseActivity implements RankTypeContract.View {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        UApp.actionEvent(this, "11_B_rank_list");
    }
    ...
    public void initEvent() {
        ...
        searchView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                UApp.actionEvent(RankTypeActivity.this, "11_A_search_btn");
            ...
            public void onClick(View v) {
                ...
                if (item instanceof TreeItemGroup) {
                    if (((TreeItemGroup) item).isExpand()) {
                        ...
                        UApp.actionEvent(RankTypeActivity.this, "11_A_row_expand_btn");
                    } else {
                        ((TreeItemGroup) item).setExpand(true);
                        ((TreeItemGroup) item).notifyExpand();
                        iv_more.setImageResource(R.drawable.ic_home_arrow_rg);
                        UApp.actionEvent(RankTypeActivity.this, "11_A_row_shrink_btn");
                    }
                }
            }
        });
        mHeaderAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickLitener() {
            ...
            public void onItemClick(ViewHolder viewHolder, BaseItem baseItem, int itemPosition) {
                if (baseItem instanceof TreeItemGroup) {
                    ...
                    if (((RankTypeInfo) baseItem.getData()).isTop()) {
                        msg.arg1 = viewHolder.itemView.getTop() + DensityUtils.dip2px(16f);
                    } else {
                    ...
                    if (baseItem.getData() instanceof RankTypeInfo.RankingsEntity) {
                        UApp.actionEvent(RankTypeActivity.this, "11_A_specific_rank_btn");
                        RankTypeInfo.RankingsEntity info = (RankTypeInfo.RankingsEntity) baseItem.getData();
                        String title = info.getTitle();
                        Intent toRank = new Intent(RankTypeActivity.this, RankActivity.class);
        ...
        final TextView tv_type_name = findViewById(R.id.tv_type_name);
        //实现悬浮停靠效果
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (count > 0 && scrollPos < count) {
                    ...
                    if (rankTypeAdapter.getItemManager() != null
                        ...
                        if (view != null) {
                            int top = view.getTop();
                            if (((RankTypeInfo) item.getData()).isTop()) {
                                top = top - DensityUtils.dip2px(16f);
                            }
                            if (top < mSuspensionHeight) {
                                mSuspensionBar.setY(-(mSuspensionHeight - top));
                            } else {
        ...
        if (mCurrentPosition < rankTypeAdapter.getItemCount()) {
            ...
            if (item instanceof TreeItemGroup) {
                if (item.getData() instanceof RankTypeInfo) {
                    ...
                    DisplayImageUtils.displayCircleImage(this,
                        ((RankTypeInfo) item.getData()).getGroupIcon(), iv_type);
                }
                if (((TreeItemGroup) item).isExpand()) {
                    mSuspensionBar.setVisibility(View.VISIBLE);
                    iv_more.setImageResource(R.drawable.ic_arrow_down_gray);
    ...
    public void showRankTypes(List<RankTypeInfo> data, int count) {
        if (rtP != null) {
            ...
            rankTypeTreeItems.clear();
            rankTypeTreeItems.addAll(ItemFactory.createTreeItemList(data,
                RankTypeOneTreeItemParent.class, null));
            rankTypeAdapter.setDatas(rankTypeTreeItems);
            mHeaderAdapter.addHeaderView(searchView);
            rclv_ranktype.setAdapter(mHeaderAdapter);
            searchView.getViewTreeObserver()
