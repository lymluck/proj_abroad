...
public class CourseContentFragment extends UIFragment implements CourseContentContract.View {
    ...
    protected View getLayoutView() {
        return mActivity.getLayoutInflater().inflate(
                R.layout.fragment_course_content, null);
    }
    ...
    //记录播放课程的进度
    private void recordePlayUrl() {
        if (mActivity != null) {
            courseP.recordPlayUrl(courseId, lastSectionId,
                    ((CourseDetailActivity) mActivity).getPlayer().getDuration() + "", (nowClickTime - lastClickTime) + "");
        }
    }
