...
public class MyChooseSchoolActivity extends UIActivity implements MySchoolContract.View {
    ...
    private int[] Pos = {-1, -1}; //保存当前item坐标的数组
    ...
    protected void onResume() {
        super.onResume();
        String targetCountryInfo = (String) SPCacheUtils.get("target_countryInfo", ParameterUtils.CACHE_NULL);
        if (!targetCountryInfo.equals(ParameterUtils.CACHE_NULL)) {
                    ...
                    if (school != null) {
                        ...
                        if ("COUNTRY_226".equals(targetCountryId)
                            ...
                            if (!targetCountryId.equals(country_id)) {
                                isNotTarget = true;
                                schoolName.setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.ic_myschool_err, 0);
                            } else {
                                schoolName.setCompoundDrawablesWithIntrinsicBounds(0, 0, 0, 0);
                            }
                        } else {
        ...
        mAdapter.setOnItemClickListener(new CommonAdapter.OnItemClickListener() {
            ...
            public void onItemClick(View view, RecyclerView.ViewHolder holder, int position) {
                if (position < schoolInfo.size()) {
                    ...
                    params.putString("id", JSON.parseObject(info.getSchool()).getString("id"));
                    Intent toMoreDetails = new Intent(MyChooseSchoolActivity.this, SchoolDetailActivity.class);
    ...
    @Override
    public void getMySchoolSuccess(int school_len) {
        if (myInfoP != null) {
            swipeRefreshLayout.setRefreshing(false);
            if (school_len > 0) {
                isNotTarget = false;
