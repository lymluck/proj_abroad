package com.smartstudy.qrcode.qrdecode;
...
public class DecodeEntry {
    ...
    public static String decodeFromFile(String filename, BarcodeFormat barcodeFormat) {
        ...
        scanBitmap = BitmapFactory.decodeFile(filename, options);
        return getPixelsByBitmap(scanBitmap, barcodeFormat, true);
    }

    public static String getPixelsByBitmap(Bitmap bitmap, BarcodeFormat barcodeFormat, boolean recycle) {
        if (bitmap == null) {
            return null;
        }
